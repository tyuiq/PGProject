<template>
  <div class="el-conten">
    ...
    <div class="el-table">
      ...
      <div class="el-fy">
        <el-pagination background layout="total, prev, pager, next" :total="15"></el-pagination>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    addList(id) {
      this.$router.push({ path: "/encyclopediasdetail/add/" + id });
    },
    detailsList(id) {
      this.$router.push({ path: "/encyclopediasdetail/details/" + id });
    },
    editList(id) {
      this.$router.push({ path: "/encyclopediasdetail/edit/" + id });
    },
    getList() {
      api
    ...
    maxSlice(v){ // 控制字符串显示长度
      return v.length > 18 ? v.slice(0, 10) + '...' : v;
    }
  }
};
</script>
